<script lang="ts">
	...
	async function uploadProfileImage() {
		...
		try {
			...
			const { data: urlData } = supabase.storage.from('profile_images').getPublicUrl(filePath);
					...
					<CardContent class="space-y-6">
						<div class="flex flex-col items-center gap-4 sm:flex-row sm:gap-8">
							<div class="relative">
								...
								<label
									for="profile-image"
									class="absolute -bottom-2 -right-2 cursor-pointer rounded-full bg-primary p-1.5 text-primary-foreground transition-colors hover:bg-primary/80"
								>
						...
						<div class="space-y-4">
							{#if updateSuccess}
								<div
									class="rounded-md bg-green-50 p-3 text-sm text-green-800 dark:bg-green-900/30 dark:text-green-400"
								>
							...
							{#if updateError}
								<div
									class="rounded-md bg-red-50 p-3 text-sm text-red-800 dark:bg-red-900/30 dark:text-red-400"
								>
									{updateError}
								</div>
							{/if}
