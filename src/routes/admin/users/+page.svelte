					...
					<table class="w-full border-collapse">
						<thead>
							<tr class="bg-muted/50">
								...
								<th class="p-2 text-left font-medium">Last Sign In</th>
								<th class="p-2 text-center font-medium">Templates</th>
								<th class="p-2 text-center font-medium">Categories</th>
								<th class="p-2 text-center font-medium">Admin</th>
							...
							{#each users as user}
								<tr class="border-b hover:bg-muted/20">
									...
									<td class="p-2 text-center">
										<span
											class="inline-flex items-center rounded-full bg-blue-100 px-2.5 py-0.5 text-xs font-medium text-blue-800 dark:bg-blue-800/20 dark:text-blue-300"
										>
											{user.template_count}
										</span>
									</td>
									<td class="p-2 text-center">
										<span
											class="inline-flex items-center rounded-full bg-purple-100 px-2.5 py-0.5 text-xs font-medium text-purple-800 dark:bg-purple-800/20 dark:text-purple-300"
										>
											{user.category_count}
										</span>
									</td>
									<td class="p-2 text-center">
										{#if user.is_admin}
											<span
												class="inline-flex items-center rounded-full bg-green-100 px-2.5 py-0.5 text-xs font-medium text-green-800 dark:bg-green-800/20 dark:text-green-300"
											>
												Admin
											</span>
										{:else}
											<span
												class="inline-flex items-center rounded-full bg-gray-100 px-2.5 py-0.5 text-xs font-medium text-gray-800 dark:bg-gray-800/20 dark:text-gray-300"
											>
												User
											</span>
										{/if}
...
<Dialog.Root bind:open={editDialogOpen}>
	<Dialog.Content>
		<Dialog.Header>
			...
			<Dialog.Description>Update user permissions</Dialog.Description>
		</Dialog.Header>
		...
		<div class="space-y-4 py-4">
			...
			<div class="flex items-center space-x-2">
				...
				/>
				<label for="is-admin" class="text-sm font-medium"> Administrator </label>
			</div>
		</div>

		<Dialog.Footer>
			<Button variant="outline" on:click={() => (editDialogOpen = false)}>Cancel</Button>
			<Button type="button" disabled={saving} on:click={saveUser}>
				{#if saving}
...
<Dialog.Root bind:open={deleteDialogOpen}>
	<Dialog.Content>
		...
		<div class="space-y-4 py-4">
			...
			<div
				class="flex items-center space-x-2 rounded-lg border border-yellow-200 bg-yellow-50 p-3 dark:border-yellow-800 dark:bg-yellow-900/20"
			>
		...
		<Dialog.Footer>
			<Button variant="outline" on:click={() => (deleteDialogOpen = false)} disabled={deleting}>
				Cancel
			</Button>
			<Button type="button" variant="destructive" disabled={deleting} on:click={deleteUser}>
				{#if deleting}
					<Icon icon="mdi:loading" class="mr-2 h-4 w-4 animate-spin" />
					Deleting...
				{:else}
