			...
			importTimeout = setTimeout(
				() => {
					if (importing) {
						importing = false;
						importError =
							'Import timed out after 5 minutes. Please try with a smaller file or check your connection.';
						importProgress = 0;
						importCurrentRow = 0;
						importTotalRows = 0;
					}
				...
				try {
					...
					const { data: template, error: templateError } = await supabase
						...
						.insert({
							...
							featured:
								rowData['Featured']?.toLowerCase() === 'yes' ||
								rowData['Featured']?.toLowerCase() === 'true'
						})
						.select()
						.single();
...
<div>
	<div class="mb-4 flex flex-col gap-4">
		<h1 class="text-2xl font-bold sm:text-3xl">Template Directory Admin</h1>
		<div class="flex w-full flex-col gap-2 sm:justify-end md:flex-row">
			<Button on:click={() => goto('/admin')} variant="outline" class="w-full sm:w-auto">
				<Icon icon="mdi:arrow-left" class="mr-2 h-4 w-4" />
				Back to Dashboard
			</Button>
			<Button
				on:click={() => (importExportDialogOpen = true)}
				variant="outline"
			...
			<div class="mt-4">
				<Tabs.Content value="templates">
					{#if templates.length === 0}
						<div class="rounded-md border bg-muted/20 py-12 text-center">
							<h2 class="mb-2 text-xl font-medium">No templates</h2>
										...
										<div class="flex items-start justify-between">
											...
											<div class="flex gap-1">
												<Button
													...
													on:click={() => toggleFeatured(template)}
												>
												...
												<Button
													...
													variant="outline"
													title="Edit"
													on:click={() => goto(`/admin/directory/${template.id}`)}
												>
												...
												<Button
													...
													variant="destructive"
													title="Delete"
													on:click={() => confirmDeleteTemplate(template)}
												>
				...
				<Tabs.Content value="categories">
					{#if categories.length === 0}
						<div class="rounded-md border bg-muted/20 py-12 text-center">
							<h2 class="mb-2 text-xl font-medium">No categories</h2>
							<p class="mb-4 text-muted-foreground">
...
<Dialog.Root bind:open={newTemplateDialogOpen}>
	<Dialog.Content class="max-w-3xl">
		...
		<Dialog.Footer>
			<Button variant="outline" on:click={() => (newTemplateDialogOpen = false)}>Cancel</Button>
			<Button
				type="button"
...
<Dialog.Root bind:open={editCategoryDialogOpen}>
	<Dialog.Content>
		...
		<Dialog.Footer>
			<Button variant="outline" on:click={() => (editCategoryDialogOpen = false)}>Cancel</Button>
			<Button
				type="button"
...
<Dialog.Root bind:open={deleteTemplateDialogOpen}>
	<Dialog.Content>
		...
		<Dialog.Footer>
			...
			<Button variant="destructive" on:click={deleteTemplate}>Delete</Button>
		</Dialog.Footer>
	</Dialog.Content>
</Dialog.Root>
	...
	<Dialog.Content>
		...
		<div class="py-4">
			...
			<p class="mt-2 text-sm text-muted-foreground">
				Note: Categories that contain templates cannot be deleted. You must first move or delete the
				templates.
			</p>
		</div>
		...
		<Dialog.Footer>
			...
			<Button variant="destructive" on:click={deleteCategory}>Delete</Button>
		</Dialog.Footer>
	</Dialog.Content>
</Dialog.Root>
		...
		<div class="space-y-6 py-4">
			...
			<div class="border-t pt-6">
				...
				<div class="space-y-3">
						...
						<div class="space-y-3">
							<div class="flex items-center justify-between">
								...
								<span class="text-sm text-muted-foreground"
									>{importCurrentRow} / {importTotalRows}</span
								>
							</div>
							...
							<div class="flex justify-center">
								<Button variant="outline" size="sm" on:click={cancelImport}>
					...
					{:else}
						<Button on:click={importFromCSV} disabled={!importFile} class="w-full">
