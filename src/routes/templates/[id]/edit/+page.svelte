			...
			if (content !== originalContent) {
				...
				if (variableMatches) {
					...
					if (newVars.length > 0) {
						...
						const { error: variablesError } = await supabase.from('variables').insert(variables);
					...
					const deletedVars = Array.from(existingVarNames).filter(
						(name) => !uniqueVars.includes(name)
					);
...
<svelte:head>
	<title
		>{template?.title
			? `Edit ${template.title} | Prompt Templates`
			: 'Edit Template | Prompt Templates'}</title
	>
</svelte:head>
...
<div class="space-y-4">
	...
	{:else if template}
		<div class="space-y-3">
			...
			<div>
				<h1 class="text-2xl font-bold tracking-tight sm:text-3xl">Edit Template</h1>
			</div>
		</div>
			...
			<CardContent class="p-4 sm:p-6">
				<form on:submit|preventDefault={handleSubmit} class="space-y-4 sm:space-y-6">
					...
					<div class="space-y-2">
						...
						<div class="w-full rounded-md border bg-background p-0 text-foreground">
							<select
								id="category"
					...
					<div class="flex flex-col gap-3 sm:flex-row sm:justify-between">
						...
						<Button type="submit" disabled={saving} class="order-1 w-full sm:order-2 sm:w-auto">
